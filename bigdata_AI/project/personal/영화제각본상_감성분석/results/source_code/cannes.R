...
# 감성분석 결과 graph 출력
graph<-emotion_analysis_graph(cannes_corpus)
...
library(ggplot2)
ggplot(gc,aes(x=reorder(sentiment,word_count),y=word_count))+geom_bar(stat='identity',fill='blue')+coord_flip()+ggtitle('cannes sentiments')
...
## 3) wordcloud 생성
library(wordcloud)
wordcloud(cannes_corpus,min.freq=10,max.words=80,
          scale=c(3,0.2),
          rot.per=0.1, 
          random.color=T,
